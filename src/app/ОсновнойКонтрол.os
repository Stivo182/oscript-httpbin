...
&Отображение("./src/app/view/templates/index.html")
&ТочкаМаршрута("/")
&GET
...
&Отображение("./src/app/view/templates/moby.html")
&ТочкаМаршрута("html")
&GET
...
Процедура ТочкаHeaders(Запрос, Ответ) Экспорт
	...
	Данные = Помощник.ПолучитьДанныеЗапроса(Запрос, , , "headers");
	Помощник.ЗаполнитьОтветJson(Ответ, Данные);
...
Процедура ТочкаRedirect(ПараметрыЗапросаИменные, Ответ, Количество) Экспорт
	...
	ПередаватьАбсольныйПуть = НРег(ПараметрыЗапросаИменные.Получить("absolute")) = "true";

	Redirect(Количество, ПередаватьАбсольныйПуть, Ответ);
...
&Отображение("./src/app/view/templates/forms-post.html")
...
Процедура ТочкаBasicAuth(Запрос, Ответ, ИмяПользователя, Пароль) Экспорт
	...
	Если ДанныеАутентификации.ИмяПользователя = ИмяПользователя
...
&Отображение("./src/app/view/templates/UTF-8-demo.txt")
&ТочкаМаршрута("encoding/utf8")
&GET
Процедура ТочкаEncodingUTF8(Запрос, Ответ, Секунд) Экспорт
...
&Отображение("./src/app/view/templates/sample.xml")
&ТочкаМаршрута("xml")
&GET
...
Процедура ПриСозданииОбъекта(&Пластилин("ПомощникПодготовкиОтветов") ПомощникПодготовкиОтветов)
	...
	КаталогШаблонов = "./src/app/view/templates";

КонецПроцедуры

Процедура Redirect(Количество, ПередаватьАбсольныйПуть, Ответ)
	...
	Иначе
		Путь = СтрШаблон("/%1-redirect/%2", 
			?(ПередаватьАбсольныйПуть, "absolute", "relative"),
			Формат(Количество - 1, "ЧГ="));	
	КонецЕсли;

	Если ПередаватьАбсольныйПуть Тогда
